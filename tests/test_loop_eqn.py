...
# ----------------------------------------------------------------------
# Partial-unknown Var metadata (#136 Tier A)
# ----------------------------------------------------------------------


def test_var_over_metadata_is_stored():
    """``Var(name, init, over=Set(...))`` accepts an IndexSet keyword
    and surfaces it on the instance. ``over`` is plain metadata at this
    tier — the engine still treats the Var as length ``len(init)``."""
    from Solverz import Set
    nb = 4
    pq = Set('PQ', np.array([1, 2], dtype=int))
    v = Var('Vm_pq', np.zeros(pq.size), over=pq)
    assert v.over is pq
    # Identity sets work too — ``over=`` doesn't require a non-identity set.
    bus = Set('Bus', nb)
    v_full = Var('Vm', np.zeros(nb), over=bus)
    assert v_full.over is bus


def test_var_over_rejects_non_indexset():
    """The ``over=`` keyword is restricted to ``Set`` / ``IndexSet`` —
    a bare list / int / Param is a usage error."""
    import pytest
    with pytest.raises(TypeError, match='Set / IndexSet'):
        Var('x', np.zeros(3), over=[0, 1, 2])
    with pytest.raises(TypeError, match='Set / IndexSet'):
        Var('x', np.zeros(3), over=3)


def test_var_over_size_validated_at_build_time():
    """``Model.create_instance`` checks that the Var's materialised
    initial value has length ``over.size``. Too short or too long
    initial values raise a clear ``ValueError`` naming the Var, the
    set, both sizes."""
    import pytest
    from Solverz import Set
    nb = 4

    # Happy path: size matches.
    m = Model()
    m.Bus = Set('Bus', nb)
    m.Vm = Var('Vm', np.zeros(nb), over=m.Bus)
    m.eqn = Eqn('f', m.Vm[0] - 1.0)
    spf, y0 = m.create_instance()
    assert y0.array.shape == (nb,)

    # Mismatched size — too short.
    m2 = Model()
    m2.Bus = Set('Bus', nb)
    m2.Vm = Var('Vm', np.zeros(nb - 1), over=m2.Bus)
    m2.eqn = Eqn('f', m2.Vm[0] - 1.0)
    with pytest.raises(ValueError, match=r"length 3 doesn't match set size 4"):
        m2.create_instance()

    # Mismatched size — too long.
    m3 = Model()
    m3.PQ = Set('PQ', np.array([1, 2], dtype=int))
    m3.Vm_pq = Var('Vm_pq', np.zeros(5), over=m3.PQ)
    m3.eqn = Eqn('f', m3.Vm_pq[0] - 1.0)
    with pytest.raises(ValueError, match=r"length 5 doesn't match set size 2"):
        m3.create_instance()


def test_var_over_default_is_none():
    """Backwards compatibility: existing ``Var(name, init)`` callsites
    don't gain an ``over`` requirement. The attribute is ``None`` by
    default so consumers can ``if var.over is not None`` to detect it.
    """
    v = Var('x', np.zeros(3))
    assert v.over is None
